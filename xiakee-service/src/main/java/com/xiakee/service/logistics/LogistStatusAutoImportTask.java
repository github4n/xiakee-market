...
public class LogistStatusAutoImportTask {
				...
				if (beans != null) {
					for (BoxnoBean bean : beans) {
						...
						if (TransferTypeEnum.BAIWEI.toCode() == bean.getTransferId()) {
							...
							if (boxno != null) {
								setLogistNode(boxno.getTime(), boxno.getMessage(), orderid, nodeIndex, TransferTypeEnum.BAIWEI,bean.getInfoId());
							}
							...
							if (nodeIndex - 1 > LogistNodeEnum.EXPRESSNO.toCode() && nodeIndex - 1 < 20) {
								...
								if (udfexData != null) {
									setLogistNode(udfexData.getActionDateTime(), udfexData.getActionDesc(), orderid, udfexData.getNodeIndex(), TransferTypeEnum.UDFEX,bean.getInfoId());
								}
							} else {
								// 新的物流节点
								...
								if (infoDatas != null && infoDatas.size() > 0) {
									setLogistNode(infoDatas, nodeIndex - 1, orderid,bean.getInfoId());
								}
							}
						}
					}
	...
	private void setLogistNode(List<UdfexDetailBean> infoDatas, int nodeIndex, String orderid,Long infoId) {
	...
	private boolean setLogistNode(String time, String message, String orderid, int nodeIndex, TransferTypeEnum transType ,Long infoId) {
