...
public class QrCodeUtil {
	...
	private static final String RESOURCE_PATH = "qrcode_logo.jpg";
	...
	public static BufferedImage encode(String content, int width, int height) {
		...
		try {
			image = genBarcode(content, width, height);
		} catch (Exception e) {
			log.error("推广链接二维码生成失败",e);
		}
		return image;
	}
	...
	private static BufferedImage genBarcode(String content, int width, int height, String srcImagePath)
		...
		// 读取源图像
		BufferedImage scaleImage = scale(IMAGE_WIDTH, IMAGE_HEIGHT, true);
	...
	private static BufferedImage scale(int height, int width, boolean hasFiller)
			throws IOException {
		double ratio = 0.0; // 缩放比例
